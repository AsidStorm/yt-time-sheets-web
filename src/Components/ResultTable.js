    ...
    return <Fragment>
        ...
        <Grid container spacing={2}>
            ...
            <Grid size={{xs: 12, md: 2}}>
                <Button fullWidth color="info" variant="outlined" onClick={() => { showCharts(); pushAnalytics("chartsButtonClick"); }}>{t('common:button.charts')}</Button>
            </Grid>
            ...
            <Grid size={12}>
                {useVirtuoso && <Paper style={{ height: 670, width: '100%'}}>
                    <TableVirtuoso
                        context={{ rows }}
                        data={rows}
                        itemContent={(index, row) => <ResultTableRowContent row={row} index={index} setInsightRow={setInsightRow} setInsightDialog={setInsightDialog} />}
                    ...
                    <Table stickyHeader className={classes.table} style={{ tableLayout: "fixed" }}>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell align="right" sx={{minWidth: 120, width: 120}}>{t('common:total')}</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {rows.map((row, index) => <TableRow sx={rowSx(row, row.realIndex)} key={`table-row-${index}`}>
                                <ResultTableRowContent row={row} index={index} setInsightRow={setInsightRow} setInsightDialog={setInsightDialog} />
                            </TableRow>)}
                        </TableBody>
                    </Table>
                </TableContainer>}
